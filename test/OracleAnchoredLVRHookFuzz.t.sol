...
contract OracleAnchoredLVRHookFuzzTest is Test, Deployers {
    ...
    function testFuzz_previewSwapFee_positiveGapIsDirectionallySymmetric(uint16 rawGapTicks) public {
        uint24 gapTicks = uint24(bound(rawGapTicks, 1, 800));

        oracle.setLatestPrice(_priceWadAtTick(int24(uint24(gapTicks))), block.timestamp);
        ...
        uint24 diff =
            feeWhenBuyingToken0 > feeWhenSellingToken0 ? feeWhenBuyingToken0 - feeWhenSellingToken0 : feeWhenSellingToken0 - feeWhenBuyingToken0;
        assertLe(diff, 1);
    }
    ...
    function testFuzz_previewSwapFee_toxicSurchargeIsMonotoneInPositiveGap(uint16 rawGapA, uint16 rawGapB) public {
        uint24 gapA = uint24(bound(rawGapA, 1, 800));
        uint24 gapB = uint24(bound(rawGapB, 1, 800));

        if (gapA > gapB) (gapA, gapB) = (gapB, gapA);

        hook.setConfig(key, _defaultConfig(BASE_FEE, LPFeeLibrary.MAX_LP_FEE, LATENCY_SECS, CENTER_TOLERANCE_TICKS));

        oracle.setLatestPrice(_priceWadAtTick(int24(uint24(gapA))), block.timestamp);
        (, uint24 feeA,,) = hook.previewSwapFee(key, false);

        oracle.setLatestPrice(_priceWadAtTick(int24(uint24(gapB))), block.timestamp);
    ...
    function testFuzz_previewSwapFee_revertsWhenCapFallsBelowComputedFee(uint16 rawGapTicks) public {
        uint24 gapTicks = uint24(bound(rawGapTicks, 1, 800));
        hook.setConfig(key, _defaultConfig(BASE_FEE, LPFeeLibrary.MAX_LP_FEE, LATENCY_SECS, CENTER_TOLERANCE_TICKS));
        oracle.setLatestPrice(_priceWadAtTick(int24(uint24(gapTicks))), block.timestamp);
        ...
        hook.setConfig(key, _defaultConfig(BASE_FEE, requiredFee - 1, LATENCY_SECS, CENTER_TOLERANCE_TICKS));
    ...
    function testFuzz_minWidthTicks_isMonotoneInSigma(uint64 rawSigmaLow, uint64 rawSigmaHigh) public {
    ...
    function testFuzz_minWidthTicks_isMonotoneInLatency(uint16 rawLatencyLow, uint16 rawLatencyHigh) public {
    ...
    function testFuzz_beforeAddLiquidity_acceptsAtCenterToleranceBoundary(uint8 rawOffsetUnits) public {
    ...
    function testFuzz_beforeAddLiquidity_rejectsJustOutsideCenterTolerance(uint8 rawOffsetUnits) public {
        uint32 offsetUnits = uint32(bound(uint256(rawOffsetUnits), 0, 10));
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                ...
                abi.encodeWithSelector(OracleAnchoredLVRHook.OffCenter.selector, midpoint, int24(0)),
    ...
    function _defaultConfig(uint24 baseFee, uint24 maxFee, uint32 latencySecs, uint32 centerTolTicks)
        internal
        view
