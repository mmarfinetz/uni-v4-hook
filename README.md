...
- LP-facing yield framing ([reports/lp_apr_uplift.md](reports/lp_apr_uplift.md)): against each pool's mid-study TVL, the recommended cell recovered `184` to `264` bps of TVL above the fixed-fee v3 baseline in the October month on WETH/USDC, WBTC/USDC, and UNI/WETH, and `2,069` bps on LINK/WETH, whose stale losses are dominated by the Oct 10-11 dislocation. October 2025 is a high-volatility month, so annualized values in that table are upper bounds, not expected yields.
