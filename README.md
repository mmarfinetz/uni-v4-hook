...
- Selectivity (observed-flow replay): the hook-based auction rule improves LP net in `28` of `54` windows, leaves `26` unchanged, and worsens none, with a `0.98%` trigger rate versus `5.82%` for the broad all-stale rule. **Caveat (2026-07-28):** these two rates come from the replay path, which was affected by the price-convention bug fixed in [docs/methodology_limitations.md](docs/methodology_limitations.md#2b); the selective rate is likely overstated and this study is pending a re-run. On the re-run October data the selective rule fires on `0.18%` of swaps versus `4.83%` broad eligibility. The direction of the claim (far more selective, none worsened) is unchanged.
