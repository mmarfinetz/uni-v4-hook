...
\begin{abstract}
Liquidity providers in automated market makers lose value when a pool price is stale relative to a faster reference market. We study a Uniswap v4 hook that detects stale pool states with an external oracle, charges toxic repricing flow with a square-root fee rule derived from one-step loss-versus-rebalancing (LVR), and applies a permissionless Dutch-auction concession. On an observed-flow replay covering \EvidenceAblationWindows{} windows across seven Uniswap v3 pools, the selective policy improves LP uplift versus a broad all-stale auction in \EvidenceAblationImproved{} windows, leaves \EvidenceAblationUnchanged{} unchanged, and worsens \EvidenceAblationWorsened{}. Its mean window trigger rate is \EvidenceSelectiveTriggerPct\%, versus \EvidenceBroadTriggerPct\% for the broad policy. Volatility is measurable in \EvidenceMeasuredAblationWindows{} windows, all classified normal; \EvidenceUnmeasurableAblationWindows{} are excluded rather than assigned a default regime. A separate October 2025 corpus contains \EvidenceOctoberNormal{} measured normal and \EvidenceOctoberStress{} measured stress windows at a 100\% annualised-volatility threshold. Because recapture is bounded by oracle freshness, we also compare with a faster CEX reference: mainnet Chainlink lags Binance by a median 21 bps, limiting recapture of CEX-measured LVR even when oracle-visible stale loss is nearly recovered.\end{abstract}
...
The observed-flow replay covers \EvidenceAblationWindows{} windows across seven Uniswap v3 pools, totalling roughly 180 hours of trading. A \textbf{window} is a contiguous block range over which informed/uninformed classification, fee accounting, and auction overlays are evaluated on the same swap stream. The source manifests retain their original routine/stress sampling tags for provenance, but those tags are not observations. Regime is instead computed from annualised realized volatility of the primary reference series. Only \EvidenceMeasuredAblationWindows{} prefixes contain enough primary-feed observations; all are measured normal at the 100\% threshold, while \EvidenceUnmeasurableAblationWindows{} remain unclassified. The observed-flow ablation therefore does not establish stress-regime generalisation. The October 2025 calibration grid is the separate regime check: it covers WETH/USDC, WBTC/USDC, LINK/WETH, and UNI/WETH across \EvidenceOctoberWindows{} four-hour windows (31 per pool), 89{,}220 swap samples, and \EvidenceOctoberOracleUpdates{} oracle updates, split into \EvidenceOctoberNormal{} measured normal and \EvidenceOctoberStress{} measured stress windows.

\subsection{Primary Benchmark: V4 Hook Ablation}
The primary benchmark is the observed-flow replay. It reconstructs v3 pool state, classifies observed swaps as informed or uninformed under the replay labels, replays fixed, hook, linear, and log fee policies, and overlays two auction policies on the same \EvidenceAblationWindows{} windows. Declared sampling labels are retained but never substituted for missing measured volatility.
...
The main result is that the selective auction improves the broad auction rule while preserving the base hook as a reserve. Across the \EvidenceAblationWindows{}-window observed-flow replay, the selective rule improves LP uplift versus the broad all-stale auction in \EvidenceAblationImproved{} windows, leaves \EvidenceAblationUnchanged{} unchanged, and worsens \EvidenceAblationWorsened{}.

\begin{center}
\small
\begin{tabular}{@{}lrrrrrr@{}}
\toprule
Subset & $N$ & Improved & Broad & Selective & Delta & 95\% CI \\
\midrule
Overall & \EvidenceAblationWindows & \EvidenceAblationImproved & \EvidenceBroadMeanUplift & \EvidenceSelectiveMeanUplift & \EvidenceMeanDelta & [\EvidenceDeltaCiLower,\ \EvidenceDeltaCiUpper] \\
Measured normal & \EvidenceMeasuredAblationWindows & \EvidenceMeasuredNormalImproved & \EvidenceMeasuredNormalBroadMeanUplift & \EvidenceMeasuredNormalSelectiveMeanUplift & \EvidenceMeasuredNormalMeanDelta & [\EvidenceMeasuredNormalDeltaCiLower,\ \EvidenceMeasuredNormalDeltaCiUpper] \\
\bottomrule
\end{tabular}
\end{center}

The unmeasurable subset has \EvidenceUnmeasurableAblationWindows{} windows: \EvidenceUnmeasurableImproved{} improve, \EvidenceUnmeasurableUnchanged{} are unchanged, and \EvidenceUnmeasurableWorsened{} worsen. These rows remain in the overall policy comparison but are excluded from every regime claim.

The selective rule is also materially less intrusive. The broad all-stale rule triggers on \EvidenceBroadTriggerPct\% of observed replay opportunities on average by window. The selective rule triggers on \EvidenceSelectiveTriggerPct\%. Event-weighted across \EvidenceObservedSwaps{} swaps, the selective arm triggers \EvidenceSelectiveEventTriggered{} auctions and fills \EvidenceSelectiveEventFilled{}, a \EvidenceSelectiveEventFillPct\% fill rate; \EvidenceSelectiveEventFallback{} fall back when the oracle is stale at modeled fill time. The model therefore supports reduced intervention, not perfect live liveness.
...
\subsection{The oracle-freshness ceiling on recapture}
The 99.9\% above is recapture of the \emph{oracle-visible} stale-loss: LVR is measured against the same Chainlink series the hook uses as its oracle, so the mechanism cannot, by construction, miss a gap its own oracle does not see. A real arbitrageur trades against the continuously-moving CEX price, whereas a Chainlink feed updates only on a deviation threshold or heartbeat. To measure true recapture we re-price every executed swap's stale-loss against a faster Binance 1-minute reference while keeping Chainlink as the hook oracle, over 24 stratified 2026 windows on WETH/USDC (mainnet feeds), using the same \texttt{correction\_trade} LVR function. The Chainlink feed lags the CEX by a median of $21$ bps at swap times (p90 $\approx 47$--$64$ bps), so only $\approx 78\%$ of true CEX-measured LVR is oracle-visible, and true recapture is about $78\%$ rather than $99.9\%$. The visible fraction falls to $\approx 40$--$45\%$ in the highest-volatility months and is $\approx 85\%$ in the calmest: the mechanism recaptures a smaller share of true LVR exactly when LVR is largest, because the CEX moves fastest then and the deviation-triggered feed lags most. This is a ceiling on \emph{any} oracle-anchored recapture mechanism, set by oracle freshness: the tested mainnet feeds carry a $0.5\%$ deviation threshold, whereas the Base ETH/USD feed is $0.15\%$ ($\approx 3\times$ tighter), so a Base deployment's true recapture is materially higher; a low-latency reference (Chainlink Data Streams or an aggregated oracle) behind the same interface closes the remaining gap without changing the fee law or auction.
...
Two properties of this payout are easy to over-read. First, it is heavily volatility-dependent and fat-tailed. October 2025 contains a market dislocation; a six-month 2026 re-run on WETH/USDC (24 stratified windows) gives a \emph{median} per-fill payout near \$0.003 with a mean near \$0.05, because the concession is $\approx 0.1\%$ of each fill's stale-loss and that scales with the gap. The per-fill payout should be read as a distribution, not a single mean. Second, the payout is the \emph{residual} left to the filler after LPs keep $\approx 99.9\%$; it is not the filler's total incentive. In a zero-gas backtest the auction clears at a near-zero concession because filling is free; with real gas the concession clock grows until a fill clears gas plus a margin, so a rational filler always earns at least their gas back by construction. A gas-aware re-tune (Base gas, \texttt{solver\_cost} fill gate) confirms fill rate stays $\approx 99.7\%$ and shows the concession \emph{growth} rate is second-order while the \emph{start} concession is the effective lever trading LP recapture against filler incentive.
...
\section{Conclusion}
The proposed design is best understood as a selective v4 hook improvement rather than as a comparison to fixed-fee v3. The base hook prices stale repricing flow with a square-root LVR fee. The Dutch auction adds an execution path only when that path improves LP accounting relative to the base hook. On the observed-flow replay, the selective rule improves LP uplift versus the broad all-stale auction in \EvidenceAblationImproved{} of \EvidenceAblationWindows{} windows, leaves \EvidenceAblationUnchanged{} unchanged, and worsens \EvidenceAblationWorsened{}, while reducing the mean window trigger rate from \EvidenceBroadTriggerPct\% to \EvidenceSelectiveTriggerPct\%. The fixed-fee v3 comparison remains useful as a leakage check. Stress-regime evidence comes only from the separately measured October corpus; the observed-flow ablation does not contain a measurable stress window.
...
The Solidity hook implementation, replay engine, October 2025 calibration grid, and the figure-generation scripts are released as the open-source repository \href{https://github.com/mmarfinetz/uni-v4-hook}{github.com/mmarfinetz/uni-v4-hook}. The empirical results in this paper correspond to evidence release \EvidenceReleaseId{}, frozen under \path{study_artifacts/evidence_release_2026_08_03}. Its manifest records the source revision and SHA-256 hashes for the replay code and inputs, while CI checks the paper macros and public claims against that release. Figures are regenerated from the frozen artifacts by the scripts under \path{script/}.

\section*{Acknowledgments}
The author thanks colleagues who reviewed earlier drafts for feedback on positioning and on the auction-design framing.
