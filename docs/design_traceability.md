...
| LPs keep `(1 − c)` of the surcharge; fee never falls below base fee (the 99.9% recapture ceiling is `c ≈ 0.1%` at clear) | `_feeUnits`: concession discounts only the surcharge term, base fee untouched | `test_auction_concessionCapsAtWadAndFeeFloorsAtBase`, `test_auction_benignDirectionStillPaysBaseFeeDuringAuction` |
