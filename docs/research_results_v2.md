...
This note studies an oracle-anchored Uniswap v4 hook that opens Dutch-auction repricing from the current pool-oracle stale gap rather than an absolute price threshold or oracle-movement proxy. The auction trigger is `stale_gap_bps_before >= trigger_gap_bps` and is shared by all selection rules. The hook fee remains `baseFee + alpha * (sqrt(price gap) - 1)` for informed stale-price trades, capped by `maxFee`; the auction is the repricing path around that fee floor. Here, informed repricing is the AMM version of toxic flow: flow that trades against stale quotes and is negative for LPs before fees. The grid covers 324 parameter sets per pool across four October 2025 pools, producing 1,296 rows in `reports/sensitivity_grid_combined.csv`. The recommended parameter set uses a 10 bps trigger gap, 5 bps base fee, 10 bps starting concession, 0.5 bps/sec growth, and a 2500 bps max fee. The empirically informative outcomes are that this cell maintains a 1.0 clear rate on all four pools, beats the fixed-fee V3 baseline everywhere, and does so with a far more selective trigger than the broad all-stale rule. Its 99.9000% mean cross-pool recapture should be read as the mechanical ceiling implied by the mechanism assumptions rather than an independent finding: with a single rational solver, zero gas, and captive flow, a clearing auction returns everything except the concession by construction (the near-zero 0.000002 pp cross-pool standard deviation reflects exactly this).

The observed-flow replay (re-run 2026-07-16 into `.tmp/dutch_auction_ablation_study_fixed2_20260716` with corrected reference orientation) adds a second check on the study machinery. It covers 54 windows across seven pools, reconstructs exact V3 state in every window, replays observed swaps through the fee curves, overlays the Dutch auction branch, and compares the broad all-stale auction rule against the hook-based auction rule. The hook-based rule improves LP net gain versus the broad all-stale rule in 28 windows, leaves 26 unchanged, and worsens none.

Orientation correction: the original May 2026 run fed external reference feeds (Chainlink, Pyth, Binance, market) in quote-asset-per-base-asset orientation, the reciprocal of the replay's token0-per-token1 pool convention, for the four pools whose base asset is token0 (LINK/WETH, UNI/WETH, WBTC/USDC, WBTC/WETH). On those pools the hook branch failed closed on every swap and the fixed-fee counterfactual accrued phantom LVR, so their windows were inert rather than genuinely unchanged. The study runner now inverts external reference series for those pools (`INVERTED_EXTERNAL_REFERENCE_FAMILIES`), the replay raises on windows whose smallest observed gap exceeds 20,000 bps, and the corrected re-run reproduces the same 28/26/0 selectivity split with all pools live. Against the static-fee policy the corrected replay shows LP net higher in 49 of 54 windows, unchanged in 5, lower in none (`reports/observed_flow_lp_uplift_windows.csv`).
...
When quoting results from this study, lead with the clear rate, the trigger selectivity, and the solver payout scale. The recapture percentage is determined almost entirely by the concession schedule once the clear rate is 1.0, so it measures the assumptions, not the market. The open empirical questions that could move the result are solver profitability after gas and how much benign flow routes away under the changed fee state.

The Solidity hook now implements this auction path on-chain: `beforeSwap` lazily opens a per-pool auction clock when the pre-swap stale gap reaches `triggerGapBps`, a permissionless `pokeAuction` lets solvers start the clock without swapping, and the toxic surcharge is discounted multiplicatively by the scheduled concession (capped at 100%, so the fee floors at `baseFee`). Because the concession keeps growing, fees that initially exceed `maxFee` eventually fall back under the cap, which removes the fail-closed deadlock that made the hook-only arm the negative control.
